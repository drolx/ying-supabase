<template>
    <div class="mt-12 ml-8">
        <h1>THE LIFESTYLE ARTICLES.</h1>

        <div class="ml-8">
            <div v-for="article in lifeArticles" :key="article.id" class="my-4">
        ...
        <NuxtLink to="/article/createarticle">
            <p class="mt-12">Create new Lifestyle Article here</p>
        </NuxtLink>
    </div>
</template>
...
const lifeArticles = ref([])

const getLifeArticles = async () => {
  const { data } = await supabase.from('articles').select('*').eq('category_id', '3')
  lifeArticles.value = data
}

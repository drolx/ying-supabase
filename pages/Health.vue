<template>
    <div class="mt-12 ml-8">
        <h1>THE HEALTH ARTICLES.</h1>

        <div class="ml-8">
            <div v-for="article in healthArticles" :key="article.id" class="my-4">
        ...
        <NuxtLink to="/article/createarticle">
            <p class="mt-12">Create new Health Article here</p>
        </NuxtLink>
    </div>
</template>
...
const getHealthArticles = async () => {
  const { data } = await supabase.from('articles').select('*').eq('category_id', '2')
  healthArticles.value = data
}
